...
public class Main_Q5 {
    public static void main(String[] args) throws IOException {
        ...
        for (int x = 0; x < bufferedImage1.getWidth(); x++) {
            for (int y = 0; y < bufferedImage1.getHeight(); y++) {
                ...
                double minDistance = Math.pow(pixelColor.getRed() - closestColor.getRed(), 2)
                 ...
                 double distance = Math.pow(pixelColor.getRed() - couleurs[1].getRed(), 2)
                         + Math.pow(pixelColor.getGreen() - couleurs[1].getGreen(), 2)
                         + Math.pow(pixelColor.getBlue() - couleurs[1].getBlue(), 2);
                 //On regarde de quelle couleur le pixel est le plus proche
                 if (distance < minDistance) {
                     closestColor = couleurs[1];
                 }
        ...
        ImageIO.write(bufferedImage2, "JPG", new File("./images_etudiants/nouvelleImageTransfoVertJaune.jpg"));
